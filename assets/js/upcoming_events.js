...
function crearMostrarTarjetasUp(arregloEventos, ubicacion, fechaBase) {


  let tarjetas = ""
  for (evento of arregloEventos) {

    // verificar los proximos eventos segun la fecha base informada


    if (evento.date >= fechaBase) {
      /*       considere que si la fechaBase es la misma del evento 
               este estaria sucediendo durante ese dia , es un proximo evento 
       */
...
crearMostrarTarjetasUp(data.events, contenido, data.currentDate)